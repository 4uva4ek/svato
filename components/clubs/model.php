...
/* ==================================================================================================== */
    ...
    public function getClubMembers($club_id, $role='') {
		...
		$sql = "SELECT c.user_id, c.role, u.nickname, u.login, u.status, u.logdate, p.karma, p.gender, p.imageurl as admin_avatar, u.is_deleted
				...
				LEFT JOIN cms_user_profiles p ON p.user_id = u.id
				WHERE club_id = '$club_id' $role_where AND premoderlist = ''
...
/* ==================================================================================================== */
    ...
    public function initClubMembers($club_id) {
		...
		$sql = "SELECT user_id, role, premoderlist
				FROM cms_user_clubs
		...
		if ($this->inDB->num_rows($rs)){
			...
			if (in_array('is_onmodern',$u)){
				$club_premembers_ids['all'][] = $u['user_id'];
		...
		$this->club_premembers_ids = $club_premembers_ids;
...
/* ==================================================================================================== */
   ...
   public function getClubPreMembersIds(){
		  
		return $this->club_premembers_ids['all']; 
...
/* ==================================================================================================== */
     ...
     * @return bool
     */
	public function checkUserRightsInClub($role='', $user_id, $is_onmodern){
...
/* ==================================================================================================== */
	...
	public function setClubMembersCount($club_id){
		...
		$members_count = $this->inDB->rows_count('cms_user_clubs', "club_id = '$club_id' AND premoderlist=''"); // учитываем администратора

		return $this->inDB->query("UPDATE cms_clubs SET members_count = {$members_count} WHERE id = '$club_id'");

	}
	
   public function ClubMembersCount($club_id){
...
// заявка на вступление в клуб
	public function PreAddUserToClub($club_id, $user_id, $role='member'){
		 return $this->inDB->query("INSERT IGNORE INTO cms_user_clubs (user_id, club_id, role, premoderlist) VALUES ('$user_id', '$club_id', '$role', 'is_onmodern')");
	}
	
	
		public function VoteUpdateUserToClub($club_id, $user_id){
		 
			$sql = "UPDATE cms_user_clubs SET premoderlist=''
		...
		public function PreRemoveUserFromClub($club_id, $user_id){
	...
	public function addUserToClub($club_id, $user_id, $role='member'){
		return $this->inDB->query("INSERT IGNORE INTO cms_user_clubs (user_id, club_id, role, premoderlist) VALUES ('$user_id', '$club_id', '$role', '')");
	}
	...
	public function VoteForPremember($club_id, $prem_id, $user_id){
		 
		 return $this->inDB->query("INSERT IGNORE INTO cms_user_preclubs (club_id, preuser_id, user_id) VALUES ('$club_id', '$prem_id', '$user_id')");
	 
	}
	
	// Подсчет Количество балов для заявителя
    public function VotePointsPremember($club_id, $prem_id, $user_id){
		 
 
		$sql = "SELECT id, preuser_id, user_id
				FROM cms_user_preclubs
				WHERE club_id = '$club_id' AND preuser_id = '$prem_id'";
		...
		if ($this->inDB->num_rows($rs)){
			while($u = $this->inDB->fetch_assoc($rs)){
				...
				$preuser_total_points['club_id'][] = $u['preuser_id'];
			  
			}
		}
  
		$preuser_total_points = isset($preuser_total_points['club_id']) ? count($preuser_total_points['club_id']) : 0;
		  
        return $preuser_total_points; 
		  
	}
	
 	// Смотри сколько раз проголосовал за участника данный юзер
 	public function VoteLimit($club_id, $prem_id, $user_id){
 	 
 
 	$sql = "SELECT club_id, preuser_id, user_id
 			FROM cms_user_preclubs
 			WHERE club_id = '$club_id' AND preuser_id = '$prem_id' AND user_id = '$user_id'";
	...
	public function clubSaveUsers($club_id, $list){
		...
		$premembers = $this->getClubPreMembersIds();
	...
	public function clubSaveUpdateUsers($club_id, $list){
...
/* ===========================================================================================
   ...
   public function getProblem($club_id){ 
	  ...
	  if(isset($this->clubs[$club_id])){ return $this->clubs[$club_id]; }
		...
		$sql =  "SELECT c.*, p.problems as problems, p.needs as needs, p.propos as propos, p.volhelp as volhelp
				FROM cms_clubs c
				WHERE c.id = '$club_id' LIMIT 1";
     ...
     public function updateProblem($club_id, $item){
...
========= */
		...
		// уведомление участникам клуба о новом заявителей
