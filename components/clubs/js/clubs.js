$(function(){
  clubs = {
    ...
    joinClub: function(club_id) {
	  ...
	  joinPreUser: function(club_id, preuser_id, user_id) {
      core.message('Голосування за користувача');
      $.post('/clubs/'+club_id+'/vote_to-'+preuser_id+'/vote_from-'+user_id, {}, function(data){
		if(data.error == false){
			$('#popup_message').html(data.text);
			$('#popup_progress').hide();
			$('#popup_ok').show();
			$('#popup_ok').click(function(){
				$('#popup_ok').prop('disabled', true);
				$('#popup_progress').show();
				$.post('/clubs/'+club_id+'/vote_to-'+preuser_id+'/vote_from-'+user_id, {confirm: 1}, function(data){
					if(data.error == false){
						window.location.href = data.redirect;
					}
				});
	...
	prejoinClub: function(club_id) {
      core.message(LANG_JOINING_CLUB);
      $.post('/clubs/'+club_id+'/prejoin.html', {}, function(data){
		if(data.error == false){
			$('#popup_message').html(data.text);
			$('#popup_progress').hide();
			$('#popup_ok').show();
			$('#popup_ok').click(function(){
				$('#popup_ok').prop('disabled', true);
				$('#popup_progress').show();
				$.post('/clubs/'+club_id+'/prejoin.html', {confirm: 1}, function(data){
					if(data.error == false){
						window.location.href = data.redirect;
					}
				});
