...
//    ОТображение участниов групп АТО которые уже в сообществах
    ///////
    ///

    //

    if ($do == 'show_ato_members'){
      ...
      //  $group_id = 11;
        ...
        assign('link', $link)->
        assign('pagebar', cmsPage::getPagebar($total, $page, $model->config['users_perpage'], $pagebar_link))->
        display('com_users_view_ato.tpl');

    }

//    ОТображение участниов групп АТО которые [еще не в сообществах]
	///////
	///


	if ($do == 'show_preato_members'){
		...
		if($total){
			...
			// Загружаем пользователей согласно выборки getUsers
			//$users_ato = $model->getUsersAtoClubs();

			// Загружаем пользователей согласно выборки getUsersPreAtoClubs
			//$users_all = $model->getUsers();
			$users = $model->getUsersPreAtoClubs();
		//	$users =  array_intersect($users_all, $users_ato);
		//	$users =  array_intersect($users_ato, $users_all) ;
		...
		assign('link', $link)->
		assign('pagebar', cmsPage::getPagebar($total, $page, $model->config['users_perpage'], $pagebar_link))->
		display('com_users_view_preato.tpl');
...
if ($do=='editprofile'){
	...
	if ($opt == 'edit'){
		...
		$armedlist = $model->getArmedGroups();
	...
	if ($opt == 'save'){
		...
		$profiles['residence'] = cmsCore::request('residence', 'str');
		$profiles['education'] = nl2br(cmsCore::request('education', 'str'));
		$profiles['kinscontacts'] = cmsCore::request('kinscontacts', 'str');
		$profiles['phones'] = cmsCore::request('phones', 'str');
		$profiles['armedgroup'] = cmsCore::request('armedgroup', 'str');
		$profiles['otharmedgroup'] = cmsCore::request('otharmedgroup', 'str');
		$profiles['atospecialty'] = cmsCore::request('atospecialty', 'str');
		$profiles['preatospecialty'] = cmsCore::request('preatospecialty', 'str');
		$profiles['atohelped'] = cmsCore::request('atohelped', 'str');
		$profiles['atohelping'] = cmsCore::request('atohelping', 'str');
		$profiles['atohelpedother'] = cmsCore::request('atohelpedother', 'str');
		$profiles['atohelpingother'] = cmsCore::request('atohelpingother', 'str');
