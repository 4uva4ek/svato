...
if ($do=='view'){
	...
	cmsPage::initTemplate('components', 'com_armedlist_view')->
            ...
            assign('pagebar', cmsPage::getPagebar($records, $page, $perpage, '/armedlist/%id%-%page%', array('id'=>$id)))->
            display('com_armedlist_view.tpl');
...
if ($do=='sendquest'){
	...
	if (!$is_submit || $error){
        ...
        if (IS_BILLING && $inUser->id){ cmsBilling::checkBalance('armedlist', 'add_quest'); }

		cmsPage::initTemplate('components', 'com_armedlist_add')->
                assign('catslist', $inCore->getListItems('cms_armedlist_cats', $category_id))->
                assign('user_id', $inUser->id)->
                assign('message', htmlspecialchars($inCore->request('message', 'html', '')))->
                assign('error', $error)->
                display('com_armedlist_add.tpl');
        ...
        if (IS_BILLING && $inUser->id){ cmsBilling::process('armedlist', 'add_quest'); }
        ...
        if (!$published){
            ...
            cmsActions::log('add_quest', array(
                ...
                'object_url' => '/armedlist/quest'.$quest_id.'.html',
                'object_id' => $quest_id,
                'target' => $category,
                'target_url' => '/armedlist/'.$category_id,
		...
		} else { $inCore->redirect('/armedlist/quest'.$quest_id.'.html'); }
