...
if ($do=='remind'){
    ...
    $user = $inDB->get_fields('cms_users',
            "MD5(CONCAT(id,'-',login,'-',password,'-','".$inDB->escape_string(PATH)."')) = '{$usercode}'", '*');
...
if ($do=='register'){
	...
	if($group_id == 9 || $group_id == 2 || $group_id == 7 || $group_id == 7 || $group_id == 1 || $group_id == 8)
	{unset($group_id); cmsCore::addSessionMessage($_LANG['WRONG_GROUP'], 'error'); $errors = true;}
	
	
	$item['residence'] = cmsCore::request('residence', 'str');
	$item['education'] = nl2br(cmsCore::request('education', 'str'));
	$item['kinscontacts'] = cmsCore::request('kinscontacts', 'str');
	$item['phones'] = cmsCore::request('phones', 'str');
	$item['armedgroup'] = cmsCore::request('armedgroup', 'str');
	$item['otharmedgroup'] = cmsCore::request('otharmedgroup', 'str');
	$item['atospecialty'] = cmsCore::request('atospecialty', 'str');
	$item['preatospecialty'] = cmsCore::request('preatospecialty', 'str');
	$item['atohelped'] = cmsCore::request('atohelped', 'str');
	$item['atohelping'] = cmsCore::request('atohelping', 'str');
	$item['atohelpedother'] = cmsCore::request('atohelpedother', 'str');
	$item['atohelpingother'] = cmsCore::request('atohelpingother', 'str');
  ...
  //  if(!cmsCore::checkCaptchaCode(cmsCore::request('code', 'str'))) { cmsCore::addSessionMessage($_LANG['ERR_CAPTCHA'], 'error'); $errors = true; }
    ...
    if ($item['is_locked']){
        ...
        if ($group_id==11)$model->sendGreetsMessageAto($item['email']);
