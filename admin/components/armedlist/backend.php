...
if ($opt == 'show_item'){
    if (!isset($_REQUEST['item'])){
        if (isset($_REQUEST['item_id'])){ dbShow('cms_armedlist_quests', (int)$_REQUEST['item_id']);  }
        echo '1'; exit;
    } else {
        dbShowList('cms_armedlist_quests', $_REQUEST['item']);
        cmsCore::redirectBack();
    }
}
...
if ($opt == 'hide_item'){
    if (!isset($_REQUEST['item'])){
        if (isset($_REQUEST['item_id'])){ dbHide('cms_armedlist_quests', (int)$_REQUEST['item_id']);  }
        echo '1'; exit;
    } else {
        dbHideList('cms_armedlist_quests', $_REQUEST['item']);
        cmsCore::redirectBack();
    }
}
...
if ($opt == 'submit_item'){
    ...
    $sql = "INSERT INTO cms_armedlist_quests (category_id, pubdate, published, quest, answer, user_id, answeruser_id, answerdate)
...
if ($opt == 'update_item'){
    ...
    if (isset($_REQUEST['item_id'])) {
        ...
        $sql = "UPDATE cms_armedlist_quests
...
if ($opt == 'show_cat'){
    if(isset($_REQUEST['item_id'])) {
        $id = (int)$_REQUEST['item_id'];
        $sql = "UPDATE cms_armedlist_cats SET published = 1 WHERE id = $id";
        $inDB->query($sql) ;
        echo '1'; exit;
    }
}
...
if ($opt == 'hide_cat'){
    if(isset($_REQUEST['item_id'])) {
        $id = (int)$_REQUEST['item_id'];
        $sql = "UPDATE cms_armedlist_cats SET published = 0 WHERE id = $id";
        $inDB->query($sql) ;
        echo '1'; exit;
    }
}
...
if ($opt == 'submit_cat'){
    ...
    $sql = "INSERT INTO cms_armedlist_cats (parent_id, title, published, description)
...
if($opt == 'delete_cat'){
    if(isset($_REQUEST['item_id'])) {
        $id = (int)$_REQUEST['item_id'];
        //DELETE ITEMS
        $sql = "DELETE FROM cms_armedlist_quests WHERE category_id = $id";
        $inDB->query($sql) ;
        //DELETE CATEGORY
        $sql = "DELETE FROM cms_armedlist_cats WHERE id = $id LIMIT 1";
        $inDB->query($sql) ;
    }
    cmsCore::redirect('?view=components&do=config&opt=list_cats&id='.(int)$_REQUEST['id']);
}
...
if ($opt == 'list_cats'){
    ...
    $fields[2]['prc'] = 'cpArmedlistCatById';  $fields[2]['filter'] = 1;  $fields[2]['filterlist'] = cpGetList('cms_armedlist_cats');
    ...
    //Print table
    cpListTable('cms_armedlist_cats', $fields, $actions);
}
...
if ($opt == 'list_items'){
    ...
    //Print table
    cpListTable('cms_armedlist_quests', $fields, $actions, '', 'pubdate DESC');
}
...
<input type="hidden" name="csrf_token" value="<?php echo cmsUser::getCsrfToken(); ?>" />
    <table width="620" border="0" cellpadding="0" cellspacing="10" class="proptable">
      <tr>
        <td><strong>Категорія формування:</strong></td>
        <td width="220"><select name="category_id" id="category_id" style="width:220px">
            <?php
                if (isset($mod['category_id'])) {
                    echo $inCore->getListItems('cms_armedlist_cats', $mod['category_id']);
                } else {
                    if (isset($_REQUEST['addto'])){
                        echo $inCore->getListItems('cms_armedlist_cats', $_REQUEST['addto']);
                    } else {
                        echo $inCore->getListItems('cms_armedlist_cats');
                    }
                }
            ?>
        </select></td>
	     ...
	     <tr>
        <td><strong>Показувати в списку?</strong></td>
    ...
    <table width="507" border="0" cellspacing="5" class="proptable">
      <tr>
        <td width="377">
        <div style="margin-bottom:10px"><strong>Назва формування:</strong></div>
        <div>
            <input type="text" name="quest" id="quest" style="border:solid 1px gray;width:605px" value="<?php echo @$mod['quest'];?>"  />
        </div>			</td>
      </tr>
...
if ($opt == 'add_cat' || $opt == 'edit_cat'){
    if ($opt=='add_cat'){
        ...
        if(isset($_REQUEST['item_id'])){
            $id = (int)$_REQUEST['item_id'];
            $sql = "SELECT * FROM cms_armedlist_cats WHERE id = $id LIMIT 1";
            $result = $inDB->query($sql) ;
            if ($inDB->num_rows($result)){
               $mod = $inDB->fetch_assoc($result);
            }
        ...
        <table width="620" border="0" cellpadding="0" cellspacing="10" class="proptable">
          ...
          <tr>
            ...
            <?php if (isset($mod['parent_id']))
                  {
                        echo $inCore->getListItems('cms_armedlist_cats', $mod['parent_id']);
                  }	else {
                        echo $inCore->getListItems('cms_armedlist_cats');
                     }
            ?>
