...
<form id="sform"action="/search" method="GET" enctype="multipart/form-data" style="clear:both">
	...
	<table width="" border="0" cellspacing="0" cellpadding="3">
        ...
        <div style="position:absolute; bottom:0; right:0; font-size:10px;">
        	<a href="javascript:void(0);" onclick="$('#sform input:checkbox').prop('checked', true);$('#from_search label').addClass('selected');" class="ajaxlink">{$LANG.SELECT_ALL}</a> |
			<a href="javascript:void(0);" onclick="$('#sform input:checkbox').prop('checked', false);$('#from_search label').removeClass('selected');" class="ajaxlink">{$LANG.REMOVE_ALL}</a>
        </div>
    </div>
</form>
</div>
