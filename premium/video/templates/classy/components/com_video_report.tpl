<div id="rerport_playbackissue" class="reports_view">
    ...
    <div class="sess_messages" style="margin: 20px 0;">
      <div class="message_info">{$LANG.PLAYBACKISSUE_INFO}</div>
    </div>
...
<div class="reports_view hid" id="rerport_inappr">
    ...
    {if $user_id}
        ...
        <select name="option_inappropriate" id="option_inappropriate" class="text-input" style="width:320px;">
            ...
            {/foreach}
        </select> <input id="movie_flag_as" type="button" value="{$LANG.REPORT}"  onclick="sendInappropriate('{$movie.id}');" />
        {else}
            <div class="sess_messages" style="margin: 20px 0;">
        ...
        {/if}
    {else}
        <p style="padding:25px 0 0 0"><a href="/login">{$LANG.TO_LOGIN}</a> {$LANG.OR} <a href="/registration">{$LANG.TO_REGISTRATION}</a> {$LANG.RIGHT_NOW}!</p>
    {/if}
</div>
{literal}
