...
<div id="action-info" class="action-info">
    ...
    {if !$from}
        <h4>{$LANG.SELECT_MOVIE} <span class="regstar" title="{$LANG.THIS_REQUIRED_FIELD}">*</span></h4>
        <div class="input_val">
            <div class="flash" id="fsUploadProgress"></div>
            <div class="input_val_value" id="input_file">
                <span id="select_flash">
                    <input type="text" id="txtFileName" disabled="disabled" tabindex="1" placeholder="{$LANG.MAX_FILE_SIZE} {$upload_max_filesize}" /> <span id="spanButtonPlaceholder"></span>
                </span>
                <span id="select_file_no_flash" class="hid">
                    <input name="Filedata" type="file" disabled="disabled" tabindex="1" />
                </span>
                <input type="hidden" name="movie_id" id="hidFileID" value="" />
            </div>
        </div>
    {elseif $from=='link'}
        <h4>{$LANG.LINK_ON_MOVIE} <span class="regstar" title="{$LANG.THIS_REQUIRED_FIELD}">*</span></h4>
        <div class="input_val">
    ...
    {else}
        <h4>{$LANG.CODE_FOR_MOVIE} <span class="regstar" title="{$LANG.THIS_REQUIRED_FIELD}">*</span></h4>
        <div class="input_val">
            <div class="input_val_value">
...
<div class="submit_video">
    {if !$from}
        <input type="submit" id="btnSubmit" tabindex="15" class="save_btn btn" value="{$LANG.START_UPLOAD}" />
        <input type="submit" id="uploadNoFlash" class="save_btn hid btn" value="{$LANG.START_UPLOAD}" onclick="return uploadNative(this);" />
    {elseif $from=='link'}
        <input type="submit" id="btnImport" tabindex="15" value="{$LANG.START_IMPORT}" class="save_btn btn" onclick="startImport();return false;" />
    {else}
        <input type="submit" id="btnImport" tabindex="15" value="{$LANG.SAVE}" class="save_btn btn" onclick="saveCode();return false;" />
    {/if}
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        swfu = new SWFUpload({
            ...
            file_size_limit : "{/literal}{$upload_max_filesize}{literal}",
