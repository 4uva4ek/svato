...
<form action="/maps/<?php if ($do=='add_item') { echo 'submit_item'.$cat_id; } else { echo 'update_item'.$cat_id.'-'.$item_id; } ?>" method="post" enctype="multipart/form-data" name="addform" id="addform">
                ...
                <table border="0" cellpadding="0" cellspacing="0" width="100%" class="addr_coord" rel="<?php echo $addr_id; ?>">
                    <tr>
                        ...
                        <td width="50%">
                            ...
                            <div id="additional_cats_list">
                                <span class="item_main_cat">
                                    <?php echo $cat['title']; ?>
                                </span>
                                <?php if ($allowed_cats){ ?>
                                    <span style="padding-left:15px">
                                        <a class="ajaxlink" href="javascript:selectCats()">Дополнительные</a>
                                    </span>
                                <?php } ?>
                            </div>
                ...
                <div><?php $inCore->insertEditor('shortdesc', $item['shortdesc'], '200', '100%'); ?></div>

                <div style="margin-top:12px"><strong>Подробное описание</strong></div>
                <div><?php $inCore->insertEditor('description', $item['description'], '400', '100%'); ?></div>

                <div style="margin-top:12px"><strong>Теги объекта</strong></div>
                <div><input name="tags" type="text" id="tags" style="width:99%" value="<?php if (isset($item['id'])) { echo cmsTagLine('map', $item['id'], false); } ?>" /></div>
